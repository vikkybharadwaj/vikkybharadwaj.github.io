#!/bin/bash
#
# refresh-highlights-pr.sh — hands-off weekly refresh of the "How I operate" stats strip.
#
...
git add -A
git commit -m "chore(highlights): weekly refresh of How I operate numbers" --quiet
...
if gh pr view "$BRANCH" --json state --jq '.state' 2>/dev/null | grep -qx OPEN; then
  ...
  gh pr create --base main --head "$BRANCH" \
    --title "chore(highlights): weekly refresh of How I operate numbers" \
    --body "Automated weekly refresh of the **How I Operate** stats strip from local git history (\`scripts/build-highlights.py\`). One or more numbers changed since the last refresh — review and merge to publish.
